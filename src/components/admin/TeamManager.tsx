...
import { Plus, Trash2, Upload, Users, Edit, GripVertical, Save, X, ArrowUpDown } from "lucide-react";
import { API_ENDPOINTS, API_URL } from "@/lib/api";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { OptimizedImage } from "@/components/ui/OptimizedImage";

type Team = { id?: number; name: string; role: string; description: string | null; avatar?: string | null; display_order?: number };
...
export default function TeamManager() {
  ...
  const [tempItems, setTempItems] = useState<Team[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(API_ENDPOINTS.team);
        const data = await res.json();
        setItems(data || []);
      } catch (e) {
  ...
  const update = async (id: number, updatedData: Team) => {
    try {
  ...
  const saveReordering = async () => {
    ...
    try {
      await fetch(`${API_URL}/api/team/reorder`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-semibold">Gestione Team</h2>
        </div>
      ...
      <div className="grid gap-3">
        {items.length === 0 ? (
          <div className="text-center py-12 text-foreground/50">
            ...
            <p>Nessun membro del team ancora. Aggiungine uno!</p>
          </div>
        ) : (
